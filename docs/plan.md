...
| D22 | D16 is lifted (owner, 2026-09-21): the mobile app is updated for the teacher-role model in short packages, each checked on the emulator against QA before the owner's device test. | owner instruction |
| D23 | Chat phase C1–C4 ran on Fable workers (owner, 2026-09-21); other packages keep the agent defaults in `.claude/agents/`. | owner instruction |
| D24 | The chat socket is Spring WebSocket inside the API (`/ws/chat`, LISTEN/NOTIFY fan-out), not a Node `ws` sidecar — one container, one auth, one deploy. | cost and operability |
| D25 | Phase E (lesson creation, 2026-09-25): L1/L2/L3 are generated **concurrently**, then Again ∥ Panel — each still its own ledger step with its own deadline and budget, so Retry, the strip and the watchdog keep their meaning. Critical path drops from five model calls to two. | the owner's "these steps take much time" |
| D26 | The dashboard socket becomes the dashboard's event channel: a `notification` frame on `/ws/chat`, open to every dashboard role, while chat frames stay gated by the `chat` flag and the TEACHER role. Notifications are persisted server-side (`notifications`, V16) and the localStorage mock bell is removed. No FCM/APNs in this phase — the app has none and the request is for the teacher on the dashboard. | one socket, one reconnect loop |
| D27 ⚠ | Levels beyond three (owner: "I can not add more level"): the contract caps `Play.level` at 3 in the schema, unlock rules, app level picker and exams. Lifting it is a contract change across server, dashboard and app (E5). Planner assumption until the owner answers: cap **5**, extra levels are hand-written or generated one at a time on request, uploads still generate 3, exams stay 1/2/3/mixed. | owner decision on the cap and on whether uploads should generate more than 3 |
...
## Phase E — Lesson creation: background, faster, simpler (2026-09-25)

Owner request: (upload paths) run in the background with a notification when done, and finish sooner; (write-by-self) fewer steps, instant "save question", more levels. Findings (planner, from the code): the job is already server-side `@Async`, but L1→L2→L3→Again→Panel run in series (≥ 6 model round trips, up to 12 on validation retries); `POST /stops/{id}/from-text` (Prompt D) runs synchronously in the request while holding a DB transaction, and "Add stop" waits on it behind a modal; the lesson page and the list poll the full lesson every 2.5 s; the bell is localStorage demo data; "ready" feedback exists only while the lesson page is open; levels are capped at 3 in `Play.schema.json`, `AdminLessonService.createPlay`, `LessonStore.assemble`, `GenerationService.TARGETS`, `StopIds`, the dashboard tabs and the app's `unlockedLevels`/`Flags.topLevel`.

| Pkg | Owner | Inputs | Outputs | Acceptance test | Depends on |
|---|---|---|---|---|---|
| E1 `backend/generation-parallel` | backend | D25; `LessonPipeline`, `LessonSteps`, `StopTextService`, `GenerationService` | L1/L2/L3 run concurrently, then Again ∥ Panel (same ledger steps, deadlines, `StepBudget`, cache-first, `retryStep` unchanged, `live` set, test hooks); `from-text`, `regeneratePlay`, `regenerateStop` make the model call outside the transaction; `GET /teacher|admin/lessons/{id}/status` — a light poll body (status, currentStep, steps, per-file convert status, plays present, updatedAt); `AdminLessonService.upload` returns the job status the client can act on; `openapi.json` regenerated | `PipelineParallelTest` (fake client with latches: three Prompt B calls in flight at once, Again waits for L1, Panel for all three; `fail-once-at=generate_L2` still leaves L1/L3 done and Retry finishes); status endpoint p95 < 30 ms local; runbook "Stuck lessons" timings updated | — |
| E2 `backend/notifications` | backend | D26; C1 chat socket | V16 `notifications` (school, user, kind, title, body, link, lesson_id, read_at, created_at); `GET /me/notifications?unread&limit`, `GET /me/notifications/unread-count`, `POST /me/notifications/{id}/read`, `POST /me/notifications/read-all`; written on `LessonState` transitions to `needs_review` (skills to confirm), `review` (ready), `error`/`paused` (failed) for the lesson's creator; `notification` frame in `ChatFrame.schema.json`/`Chat.kt`, `ChatEvent` kind, bus payload, `ChatSessions.send` to the user's key; handshake admits ADMIN/MANAGERIAL/TEACHER regardless of the `chat` flag, chat frames and chat REST stay gated; `permissions.json` `notifications.read` for the three dashboard roles | MockMvc: a pipeline finishing on the fake client produces one `lesson.ready` row for the creator and none for another teacher; tenancy on the NOTIFY path (school B never receives school A's frame); read/unread round trip; `FlagAdminTest`/`PermissionsTest` green | E1 |
| E3 `dashboard/lesson-background` | dashboard | E1 status endpoint, E2 frames/REST | New lesson: create → upload → analyze chain owned by a service (not the page), "Work in background" button to the list, no pull-back navigation, upload job errors shown instead of a blind analyze; lesson page polls `/status` and reloads the full lesson only on a step transition; list polls `/status` per running row; real bell: `NotificationsService` over E2 REST + `notification` frame, unread badge, notifications page lists real rows, toast + browser `Notification` (only when the tab is hidden, permission asked on a click) with distinct copy for ready / skills to confirm / failed; socket opens for every dashboard role | Vitest for the service and the bell; Playwright `lesson-background.spec.ts` (local, fake LLM): upload a PDF, click "Work in background", wait on the list, the bell shows "Questions ready", clicking it opens the lesson at Review; existing lesson specs green | E1, E2 |
| E4a `dashboard/write-yourself-flow` | dashboard | add-stop, lesson page | After "Create and write the questions" the Add question sheet opens on the empty L1; "Save and add another"; the assistant path (`from-text`) is non-blocking — the sheet closes, the stop shows "The assistant is writing…", the call runs in a service with a 90 s client timeout and a Retry chip, and only that stop is patched (no full-lesson reload); prose edits no longer set the page-wide `busy`; Level 2 / Level 3 / Again get a visible "Add level" action in the manual flow; the 605 kB validator chunk loads only for the admin debug panel | Playwright `add-stop.spec.ts` updated: ten questions in a row without waiting on the modal; `lesson-editor.spec.ts` green; chunk not requested for a teacher (network assertion) | E1 |
| E4b `dashboard/write-yourself-instant` | dashboard | stop templates, `PUT /stops/{id}` | Structured fields on the form for the most-used types (choice, trueFalse, fillBlank, readPage, exitTicket): the client builds the stop JSON from the template and saves with one `PUT` — no model call, save < 1 s; free text stays as the assistant path | Playwright: a choice question with four options saves in one request (no `from-text` call), plays in the phone preview | E4a |
| E5 ⚠ `levels-up-to-five` | backend → dashboard → mobile (three packages) | D27, owner's answer | Contract: `Play.schema.json`/`MapResponse.schema.json` level 1–5, `createPlay` cap, `StopIds`, `Prompts.userB` briefs for L4/L5, `POST /lessons/{id}/plays/{level}/generate` (one level on request, async step); dashboard tabs from data with "Add level"; app: `unlockedLevels`, `Flags.topLevel`, `LevelSelector` labels, `LessonCompleteScreen`, `ContentRepositoriesImpl` loops | Contract tests; a 5-level manual lesson publishes, plays on the emulator to level 5, results group five levels | owner decision |

Order: E1 → E2 → E3 → E4a → E4b; E5 only on the owner's word. Reviewer after each; QA verified by the planner after each deploy.

## Status

**Phase 1: done 2026-09-16.** **Phase 2: done 2026-09-16** (server flags/themes/platform settings, app join-school/theme/gates, e2e, docs). QA runs `cb49991`; both QA schools themed. Phase 3 partly done, then replanned (2026-09-18) into N1–N6. **N1–N2 done 2026-09-19: the teacher flow is on QA** (`0b800fc`, e2e 47/0 from the planner's machine; the post-deploy job needs the `qa` secret `E2E_STAFF_PASSWORD` set to the seeded staff password). **Restyle T1–T5 done 2026-09-19** (`2c12c5a`, Lighthouse 97/100). **Change request CR1–CR5 done 2026-09-19** (`2753d2e`; Markdown-first pipeline proven on QA). **QA is the owner's acceptance environment since 2026-09-19**: `SEED_PROFILE=acceptance` (Maya math 1A+1B British, Rami english 1A American, join code `HQ0001`, no children), the automated QA e2e job parked via the repository variable `E2E_ON_QA=false` until QA is back on the full seed; the mobile parent flow passes on the emulator (`docs/reports/mobile-parent-acceptance.md`). **N4 done 2026-09-20** (`docs/reports/n4-verification.md`); the child's side of exams and results in the app still needs D16 lifted. **Mobile update started 2026-09-21 (D22)**: M1 merged; the owner runs a quick device test after each mobile package. Next: M2 exams in the app, exams-tab fan-out cleanup, N5, N6. Open: N3 web player, N4 gradebook/exams, N5, N6, N1.2b; owner decision on the mobile app vs web player; repo back to private at the end.
...
| E5 ⚠ | — | owner decision | levels up to five |
