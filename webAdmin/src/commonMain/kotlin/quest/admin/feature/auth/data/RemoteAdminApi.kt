...
class RemoteAdminApi(private val baseUrl: String, private val token: () -> String?, engine: HttpClient = HttpClient()) : AdminApi {
    ...
    private fun stepName(s: quest.api.PipelineStep) = when (s) { quest.api.PipelineStep.UPLOAD -> "upload"; quest.api.PipelineStep.ANALYZE -> "analyze"; quest.api.PipelineStep.SKILLS -> "skills"; quest.api.PipelineStep.GENERATE_L1 -> "generate_L1"; quest.api.PipelineStep.GENERATE_L2 -> "generate_L2"; quest.api.PipelineStep.GENERATE_L3 -> "generate_L3"; quest.api.PipelineStep.GENERATE_AGAIN -> "generate_again"; quest.api.PipelineStep.PANEL -> "panel" }
