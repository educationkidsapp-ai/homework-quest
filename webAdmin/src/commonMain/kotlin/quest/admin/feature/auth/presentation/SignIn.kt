...
fun SignInScreen(vm: SignInViewModel, apiBaseUrl: String) {
    ...
    Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Card(Modifier.width(420.dp)) {
            ...
            Text("Admin sign in · $apiBaseUrl", style = MaterialTheme.typography.bodySmall, color = Palette.parentInkSoft)
            Spacer(Modifier.height(20.dp))
            ErrorBanner(s.error)
            Field(s.email, { vm.dispatch(SignInContract.Intent.Email(it)) }, "Email")
            Spacer(Modifier.height(10.dp))
            OutlinedTextField(s.password, { vm.dispatch(SignInContract.Intent.Password(it)) }, Modifier.fillMaxWidth(), label = { Text("Password") }, singleLine = true, visualTransformation = PasswordVisualTransformation())
            Spacer(Modifier.height(20.dp))
            AdminButton(if (s.busy) "Signing in…" else "Sign in", { vm.dispatch(SignInContract.Intent.Submit) }, Modifier.fillMaxWidth(), enabled = !s.busy && s.email.isNotBlank() && s.password.isNotBlank())
        }
    }
}
