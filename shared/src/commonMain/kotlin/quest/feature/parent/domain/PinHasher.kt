...
/**
 * Salted, iterated SHA-256 for the 4-digit parent PIN, implemented in common Kotlin so every platform
 * hashes identically. Stored format: `v1$<salt>$<hex>`.
 */
object PinHasher {
    ...
    fun hash(pin: String, salt: String = randomSalt()): String {
        ...
        repeat(ITERATIONS - 1) { digest = sha256(digest + salt.encodeToByteArray()) }
        return "v1$$salt$" + digest.toHex()
    }
    ...
    private fun ByteArray.toHex() = joinToString("") { b -> ((b.toInt() and 0xff) or 0x100).toString(16).substring(1) }
    ...
    fun sha256(message: ByteArray): ByteArray {
        ...
        val padded = message + byteArrayOf(0x80.toByte()) + ByteArray(((56 - (message.size + 1) % 64) + 64) % 64) +
            ByteArray(8) { i -> (bitLen ushr (56 - 8 * i)).toByte() }
        val w = IntArray(64)
        for (chunk in 0 until padded.size / 64) {
            val base = chunk * 64
            for (i in 0 until 16) {
                w[i] = ((padded[base + 4 * i].toInt() and 0xff) shl 24) or ((padded[base + 4 * i + 1].toInt() and 0xff) shl 16) or
                    ((padded[base + 4 * i + 2].toInt() and 0xff) shl 8) or (padded[base + 4 * i + 3].toInt() and 0xff)
            }
