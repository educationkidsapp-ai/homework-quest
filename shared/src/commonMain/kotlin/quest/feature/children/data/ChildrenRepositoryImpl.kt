...
class ChildrenRepositoryImpl(private val api: ContentApi, private val db: Db, private val settings: SettingsStore, private val auth: AuthProvider) : ChildrenRepository {
    ...
    override suspend fun refresh(): List<Child> {
        ...
        val merged = when {
            ...
            else -> remote.also { list -> db.write { list.forEach { save(it) } } }
        }
    ...
    override suspend fun children(): List<Child> = db.read { selectChildren(uid()).executeAsList().map { it.toDomain() } }
    ...
    override suspend fun select(id: String) {
        _current.value = db.read { selectChild(id).executeAsOneOrNull()?.toDomain() }
    ...
    private fun quest.core.db.Child.toDomain() = Child(id, name, avatarColor, Curriculum.valueOf(curriculum.uppercase()), grade.toInt(), languages.split(',').filter { it.isNotBlank() })
}
