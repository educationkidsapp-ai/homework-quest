...
Token economy: read `gh pr diff <n>` and the files it touches (ranges only). CI is the gate for the full suites — read its results instead of re-running them; run only targeted tests and, at most, one throwaway probe for the riskiest claim; run a full local suite only when CI is red or absent. Re-reviews check the delta only. Keep the comment to the checklist plus one line per ❌.
