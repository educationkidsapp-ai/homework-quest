...
/** SHA-256 (FIPS 180-4) in common Kotlin — used for cache keys and the parent PIN hash. */
object Sha256 {
    fun hex(bytes: ByteArray): String = digest(bytes).joinToString("") { b -> ((b.toInt() and 0xff) or 0x100).toString(16).substring(1) }
    ...
    fun digest(message: ByteArray): ByteArray {
        ...
        val padded = message + byteArrayOf(0x80.toByte()) + ByteArray(((56 - (message.size + 1) % 64) + 64) % 64) + ByteArray(8) { i -> (bitLen ushr (56 - 8 * i)).toByte() }
        val w = IntArray(64)
        for (chunk in 0 until padded.size / 64) {
            val base = chunk * 64
            for (i in 0 until 16) w[i] = ((padded[base + 4 * i].toInt() and 0xff) shl 24) or ((padded[base + 4 * i + 1].toInt() and 0xff) shl 16) or ((padded[base + 4 * i + 2].toInt() and 0xff) shl 8) or (padded[base + 4 * i + 3].toInt() and 0xff)
